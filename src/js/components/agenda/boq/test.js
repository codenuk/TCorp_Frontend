...
class ComponentProjectTopic extends React.Component {
    ...
    componentDidMount() {
        ...
        this.getData().then(function (data) {
            ...
            current.setState({ headDetail: headDetail, boqID: headBoqId[0].id, isTokenValid: true });
            console.log(">>>>>>>>>>>>>>>>>>>>>>>headBoqId[0].id",headBoqId[0].id);
        }).catch(function (err) {
            console.log(err);
    ...
    render() {
        ...
        return (
            <div className="row">
                ...
                <Link to={`/boq/${this.props.tcorp_id}/`}><button type="submit" className="btn btn-dark mr-1">BOQ</button></Link>
