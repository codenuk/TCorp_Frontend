...
class ComponentArchiveCards extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.get(`http://vanilla-erp.com:${API_URL_DATABASE}/api/v1/projects/overview`, { headers: { 'x-access-token': token_auth } })
            .then(res => {
                ...
                this.setState({ projects: projects_ongoing, isTokenValid: true });
            })
            .catch(function (err) {
    ...
    persent(total_tasks, total_finished) {
        console.log("total_tasks, total_finished",total_tasks, total_finished)
        if (total_finished === null){
            var zero = "0%";
            return zero;
        }
        var cal_per = total_finished / (total_tasks-3) * 100;
                ...
                {this.state.projects.map((project) => (
                    <Link to={{ pathname: `/project-task/${project.tcorp_id}`, id: project.id }} className="none_line">
                        <div className="card">
                            ...
                            <h1 className="Card-setting">{current.cutNameProject(project.name_th)}</h1>
                            <div className="card-body">
                                ...
                                <div className="row">
                                    <div className="col-12">
                                        ...
                                        <p className="Class-info-status" style={{ color: "black" }}>{this.checkValue(project.value.toLocaleString())} ฿</p>
                                    </div>
                                </div>
                                <div className="row">
                                    <div className="col-5">
